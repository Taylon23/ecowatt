...
class CalculoConsumoForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['nome_personalizado', 'tipo', 'potencia', 'horas_por_dia', 'tarifa_por_kwh']
        labels = {
    ...
    def clean_horas_por_dia(self):
        ...
        if horas < 0 or horas > 24:
            raise forms.ValidationError("O valor de horas por dia deve estar entre 0 e 24.")
        return horas

    def clean_tarifa_por_kwh(self):
        tarifa = self.cleaned_data.get('tarifa_por_kwh')
        if tarifa <= 0:
            raise forms.ValidationError("A tarifa por kWh deve ser maior que zero.")
        return tarifa
